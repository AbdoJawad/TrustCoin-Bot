...
def main() -> None:
    ...
    try:
        ...
        if webhook_url:
            ...
            print(f"🚀 Starting French bot in webhook mode on port {port}")
            ...
            def health():
                ...
                return {'status': 'healthy', 'bot': 'french'}
            
            @flask_app.route('/', methods=['GET'])
            def home():
                """Home endpoint."""
                return {'message': 'TrustCoin French Bot is running!', 'status': 'active'}
            ...
            print("🔄 Starting French bot in polling mode (development)")
            app.run_polling(drop_pending_updates=True)
            
    except InvalidToken:
        print("❌ Invalid bot token. Please check your BOT_TOKEN_FR in .env file.")
    except Exception as e:
        print(f"❌ Error starting French bot: {e}")
